...
if (member.id === client.user.id)
      return message.channel.send({
        embeds: [
          new client.emb().desc(
            `<:n_cross:1191377533413904445> ${message.author}: I can't assign roles to myself`,
          ),
        ],
      });
    ...
    if (member.id === message.guild.ownerId)
      return message.channel.send({
        embeds: [
          new client.emb().desc(
            `<:n_cross:1191377533413904445> ${message.author}: You can't assign roles to the guild owner`,
          ),
        ],
      });
    ...
    if (!member.roles.cache.has(role.id)) {
      ...
      if (x) {
        return message.channel.send({
          embeds: [
            new client.emb()
              .desc(`<:n_tick:1191377412240441364> ${message.author}: **Added** ${role} to ${member.user.tag}`)]
        });
      }
      return;
    }
    ...
    if (x) {
      return message.channel.send({
        embeds: [
          new client.emb()
            .desc(`<:n_tick:1191377412240441364> ${message.author}: **Removed** ${role} from ${member.user.tag}`)]
      });
  }
