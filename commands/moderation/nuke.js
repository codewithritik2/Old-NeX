...
let ch = message.mentions.channels.first() || message.channel
   ...
   const yes = new client.button().secondary(`nuke_yes`, `Yes`, `<:n_tick:1191377412240441364>`)

    const no = new client.button().secondary(`nuke_no`, `No`, `<:n_cross:1191377533413904445>`)
...
collector.on('collect', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === `nuke_yes`) {
     interaction.update({embeds: [new EmbedBuilder().setColor(client.color).setAuthor({name: message.author.username, iconURL: message.author.displayAvatarURL({dynamic: true})}).setTimestamp().setDescription(`**<:n_tick:1191377412240441364> ${message.author}:** Successfully Nuked ${ch}`)], components: []})
      ch.clone()
      ch.delete()
    }
