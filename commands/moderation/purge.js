  ...
  execute: async (client, message, args, prefix) => {
    ...
    if (args[0] === `images` || args[0] === `files`) {
      ...
      .send({
        embeds: [
          new client.emb().desc(
            `<:n_tick:1191377412240441364> ${message.author}: Purged ${msgs.size} messages`,
          ),
        ],
      ...
      .send({
        embeds: [
          new client.emb().desc(
            `<:n_tick:1191377412240441364> ${message.author}: Purged ${msgs.size} human messages`,
          ),
        ],
      }).then((m) => setTimeout(() => m.delete(), 3000))
      .catch((e) => {});
      ...
      .send({
        embeds: [
          new client.emb().desc(
            `<:n_tick:1191377412240441364> ${message.author}: Purged ${msgs.size} bot messages`,
          ),
        ],
      }).then((m) => setTimeout(() => m.delete(), 3000))
      .catch((e) => {});
