...
module.exports = {
  ...
  execute: async (client, message, args, prefix) => {
   ...
   let ref;
    ...
    if (ref?.content?.match(/\/emojis\/(\d+)/) || message?.content?.match(/\/emojis\/(\d+)/)) {
      ...
      return message.channel.send(`<:n_tick:1191377412240441364> Successfully Added Emoji(s) : <a:${newEmoji.name}:${newEmoji.id}>`);
    } catch (error) {
      try {
        const newEmoji = await message.guild.emojis.create({
        ...
        return message.channel.send(`<:n_tick:1191377412240441364> Successfully Added Emoji(s) : <:${newEmoji.name}:${newEmoji.id}>`);
      } catch (error) {
        return message.channel.send({embeds: [ new client.emb().desc(`\`\`\`\n${error}\`\`\``)]});
            ...
            if (!args) return message.reply({ content: `<:n_cross:1191377533413904445> No emojis provided!` });
        ...
        let desc = `<:n_tick:1191377412240441364> Successfully Added Emoji(s) : `;  
      
      if (!normemojis && !animemojis) return message.reply({ content: `<:n_cross:1191377533413904445> No emojis provided!` });
  ...
  if (!id) {
   ...
   /* try {
      ...
      channel.send(`<:n_tick:1191377412240441364> Added the emoji <a:${newEmoji.name}:${newEmoji.id}>`);
    } catch (error) {
      try {
        const newEmoji = await guild.emojis.create({
          attachment: `https://cdn.discordapp.com/emojis/${id[1]}.png`,
        ...
        channel.send(`<:n_tick:1191377412240441364> Added the emoji <:${newEmoji.name}:${newEmoji.id}>`);
      } catch (error) {
